...
import { getReactThemeRenderer, type ThemeName, type ThemeHelpers } from './themes.js';
...
export function generateReact(ast: ASTNode[]): GeneratedCode {
  ...
  const themeNode = ast.find(n => n.type === 'ThemeDecl') as ThemeDecl | undefined;
...
function genUINode(node: UINode, c: GenContext): string {
  ...
  if (c.theme) {
    ...
    if (renderer) {
      const helpers: ThemeHelpers = { genExpr: (e, ctx) => genExpr(e, ctx || c), genTextContent: (e, ctx) => genTextContent(e, ctx || c), genActionExpr: (a, ctx) => genActionExpr(a, ctx || c), genUINode: (n, ctx) => genUINode(n, ctx || c), srcComment: (n) => srcComment(n) };
...
function genLayout(node: LayoutNode, c: GenContext): string {
  ...
  for (const [key, val] of Object.entries(node.props)) {
    ...
    switch (key) {
      ...
      case 'shadow': {
        const sv = unquote(v);
        if (sv === 'sm') style['boxShadow'] = '0 1px 2px rgba(0,0,0,0.1)';
        else if (sv === 'md') style['boxShadow'] = '0 4px 6px rgba(0,0,0,0.1)';
        else if (sv === 'lg') style['boxShadow'] = '0 10px 15px rgba(0,0,0,0.1)';
        break;
      }
    }
  }
